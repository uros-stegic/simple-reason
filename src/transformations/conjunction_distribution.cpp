...
Formula ConjunctionDistribution::transform(const Formula &f) const
{
    switch( f->get_type() ) {
    	...
    	case CONJUNCTION: {
    	    auto left  = static_cast<const And*>(f.get())->get_left_operand();
    	    auto right = static_cast<const And*>(f.get())->get_right_operand();
    	    if( left->get_type() > DISJUNCTION || right->get_type() > DISJUNCTION ) {
    	        throw new std::runtime_error("Cannot distribute conjunction over implication or equivalence");
    	    }
    	    if( right->get_type() == DISJUNCTION ) {
    	        return std::make_shared<Or>(
    	                    std::make_shared<And>(
    	                        left,
    	                        static_cast<const Or*>(right.get())->get_left_operand()
    	                    )->transform(*this),
    	                    std::make_shared<And>(
    	                        left,
    	                        static_cast<const Or*>(right.get())->get_right_operand()
    	                    )->transform(*this)
    	               );
    	    }
    	    else if( left->get_type() == DISJUNCTION ) {
    	        return std::make_shared<Or>(
    	                    std::make_shared<And>(
    	                        static_cast<const Or*>(left.get())->get_left_operand(),
    	                        right
    	                    )->transform(*this),
    	                    std::make_shared<And>(
    	                        static_cast<const Or*>(left.get())->get_right_operand(),
    	                        right
    	                    )->transform(*this)
    	               );
    	    }

    	    return std::make_shared<And>(left->transform(*this), right->transform(*this));
    	}
    	case DISJUNCTION: {
    	    auto left  = static_cast<const Or*>(f.get())->get_left_operand() ->transform(*this);
    	    auto right = static_cast<const Or*>(f.get())->get_right_operand()->transform(*this);
